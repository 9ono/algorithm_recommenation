...
def accuracy(y_actual, y_predict):
    ...
    '''
    result = pd.DataFrame(columns=['Precision', 'Recall', 'F1-Score', 'Accuracy', 'Support'])
    ...
    for col in y_actual.columns:
        ...
        tn = ((y_actual[col]==0) & (y_predict[col]==0)).sum()
        ...
        result.loc[col] = [precision, recall, f1_score, accuracy, support]
    ...
    result.loc['Avg/Total'] = [avg_precision, avg_recall, avg_f1_score, avg_accuracy, result['Support'].sum()]
...
def get_features_tfidf(pipeline, labels, num_features=10, prob_only=False):
    ...
    '''
    category_columns = ['math', 'dp', 'implementation', 'greedy', 'data structures',
    'sortings',  'dfs', 'graphs', 'trees', 
    'constructive algorithms', 'brute force', 'strings', 'binary search', 'number theory',
    'geometry']
...
def multi_class_predict(clf, test_X, class_to_genre_map):

    print(type(test_X))
    if isinstance(test_X, pd.Series):
        y_pred = pd.DataFrame(index=test_X.index, columns=class_to_genre_map.columns)
    else:
        y_pred = pd.DataFrame(index=range(test_X.shape[0]), columns=class_to_genre_map.columns)
